...
class TranscriptionManager:
    ...
    async def start_transcription(self, model_size, filename):
        ...
        try:
            ...
            await self.broadcast({"type": "status", "text": "Model loaded. Recording..."})
            ...
            # Auto-detect monitor/pulse device
    ...
    async def ws_handler(self, websocket):
        ...
        try:
            async for message in websocket:
                ...
                if action == "start":
                    asyncio.create_task(self.start_transcription(data.get("model", "medium"), data.get("filename", "web_transcript")))
                elif action == "stop":
                    self.is_running = False
        finally:
    ...
    async def cli_input_task(self):
        ...
        while True:
            # We use a thread-safe way to get input in an async loop
            print("\n[CLI] Enter filename to start local transcription (or use Frontend): ", end="", flush=True)
            filename = await asyncio.get_running_loop().run_in_executor(None, sys.stdin.readline)
            filename = filename.strip()
            if filename:
                print(f"[CLI] Starting local transcription for: {filename}")
                await self.start_transcription("medium", filename)
